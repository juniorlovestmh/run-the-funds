...
key_decisions:
  ...
  - Integration tests use rtf::* library items to seed rates rather than calling `rtf convert` as a seeding mechanism — avoids chicken-and-egg (seeding via `convert` would itself hit BCB).
...
**`tests/convert_demo.rs`** — 3 active integration tests + 1 `#[ignore]`d live-BCB smoke test, all shelling out to the compiled binary via `env!("CARGO_BIN_EXE_rtf")`.

Key design: tests use `rtf::...` library items to seed rates directly into the SQLite DB before CLI invocations. Zero live BCB round-trips = zero CI flakiness. `seed_rate` and `seed_pair` helpers keep the setup terse.
...
- **New S04** inserted: "Bank Sync Adapters (SimpleFIN + Pluggy)" with concrete Done criteria (SimpleFIN pulls US txns automatically, Pluggy does the same for Brazilian banks, `rtf sync` reconciles both, secrets in env vars).
