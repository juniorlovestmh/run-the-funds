...
**Goal:** Make Monarch Money the single upstream source for rtf's US financial data. Retire direct-to-bank adapters (Teller, SimpleFIN, Pluggy) and Connect scaffolding. Import Monarch's category groups + categories + tags into rtf's domain (new tags + transaction_tags tables). Build MonarchAdapter that shells out to the installed mmoney CLI for JSON, parses into rtf's domain, resolves Monarch IDs to rtf-local IDs via external_id upsert, and persists via the existing persist_batch pipeline. Nuke-and-rebuild migration (user confirmed zero attachment to current rtf DB).
**Demo:** rtf sync pulls every account/transaction/category/tag from Monarch Money via mmoney subprocess. rtf transactions list matches mmoney count. rtf spending matches Monarch's cashflow within 1%. Teller/SimpleFIN/Pluggy/Connect code removed.

## Must-Haves

- rtf sync (Monarch-only, no --provider flag) pulls every Monarch account/transaction/category/tag into rtf. Re-running is idempotent — no duplicates, updates reflect Monarch's latest. rtf transactions list count matches mmoney transactions list within ±5. rtf spending for a known month matches Monarch's cashflow within 1%. cargo build references no Teller/SimpleFIN/Pluggy symbols. cargo test all green.
...
rtf's data plane switches to Monarch-only subprocess pull. Pre-S06 backup of rtf.db taken before migration wipe. Monarch's server-side rules continue auto-categorizing new transactions; rtf mirrors the resulting state.

## Verification

- rtf sync emits MonarchSyncReport {accounts_imported, transactions_imported, categories_imported, tags_imported, duplicates_skipped, window_start, window_end}. Preserves {status, data} CLI envelope. On mmoney auth failure, surfaces 'run mmoney auth login first' guidance.
...
- [x] **T02: Tags domain + schema + repository** `est:medium`
  ...
  - Verify: cargo test: new tag_repo tests pass including CASCADE delete. rtf tags list --format json on empty DB returns empty array envelope.

- [ ] **T03: MonarchAdapter — subprocess-based mmoney ingest** `est:medium`
...
- [ ] **T04: SyncService Monarch wiring: taxonomy then accounts then transactions with ID resolution** `est:large`
  ...
  - Verify: Unit tests: empty Monarch → empty import; re-running produces 0 duplicates; new category in Monarch creates new rtf category; tag-id reference resolution; transaction category resolved via lookup. Integration-ish test with FakeMmoneyRunner backed by survey/*.json fixtures.

- [ ] **T05: CLI rewire + live demo + UAT + rollup commit** `est:medium`
